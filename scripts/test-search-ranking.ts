...
ok('weak: maus vs Di Di Mau below floor (no token hit)', titleMatchSignal('maus', mk('Di Di Mau')) < STRONG_MATCH_FLOOR)
ok('single buried word lands AT floor, not below (not falsely weak)', titleMatchSignal('blade', mk('Blood Blade Volume 2')) === STRONG_MATCH_FLOOR)
ok('correct buried single-word match (The Complete Maus) is NOT weak', titleMatchSignal('maus', mk('The Complete Maus')) >= STRONG_MATCH_FLOOR)
