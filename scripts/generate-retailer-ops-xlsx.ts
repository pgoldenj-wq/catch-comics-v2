...
const retailers = [
  ...
  },
  {
    name:          'Amazon UK',
    domain:        'amazon.co.uk',
    dbListings:    '~93 priced',
    monetised:     '✅ LIVE',
    commissionPath:'Amazon Associates tag: catchcomics-21',
    affiliateId:   'catchcomics-21',
    feedState:     'Via Rainforest API (~$0.001/call)',
    feedFid:       'N/A',
    status:        'LIVE',
    statusType:    'live',
    priority:      'P0',
    priorityType:  'p0',
    nextAction:    'npm run enrich:amazon -- --write --limit 2000 (~3.5h, ~$2)',
    owner:         'Claude',
    reviewDate:    '2026-05-22',
    notes:         'Best commission rate. Only 93 priced from ~9,100 TM-linked ISBNs. Full run = ~$9, ~16h.',
  },
  {
    name:          'Amazon US',
    domain:        'amazon.com',
...
async function main() {
  ...
  const envRows = [
    ...
    ['RAINFOREST_API_KEY',                      'Set (see .env.local)',             'Rainforest API for Amazon enrichment (~$0.001/call)'],
    ['NEXT_PUBLIC_AMAZON_UK_ASSOCIATE_TAG',     'catchcomics-21',                   '✅ Correct — UK Associates tag'],
    ['NEXT_PUBLIC_AMAZON_US_ASSOCIATE_TAG',     'catchcomics-21 ← BUG',            '❌ Wrong tag (UK tag used for US). Blank this out in Vercel until real US tag obtained'],
